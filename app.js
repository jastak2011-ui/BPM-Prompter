...
const rawEstimatorBpmReadout = document.getElementById("rawEstimatorBpmReadout");
const onsetBpmReadout = document.getElementById("onsetBpmReadout");
...
const targetAssistedBpmReadout = document.getElementById("targetAssistedBpmReadout");
const finalDisplayBpmReadout = document.getElementById("finalDisplayBpmReadout");
const estimatorAgreementReadout = document.getElementById("estimatorAgreementReadout");
const detectedBpmAvailableReadout = document.getElementById("detectedBpmAvailableReadout");
const anchorOnlyReadout = document.getElementById("anchorOnlyReadout");
const candidateRejectionReadout = document.getElementById("candidateRejectionReadout");
const rejectedHighBpmCandidatesReadout = document.getElementById("rejectedHighBpmCandidatesReadout");
const rejectedSubdivisionCandidatesReadout = document.getElementById("rejectedSubdivisionCandidatesReadout");
...
const lowBandEnergyReadout = document.getElementById("lowBandEnergyReadout");
const lowBandFluxReadout = document.getElementById("lowBandFluxReadout");
...
let spectralEstimatorBpm = null;
let targetAssistedBpm = null;
let finalDisplayBpm = null;
let lastEstimatorUpdateTime = 0;
let lastDisplayUpdateTime = 0;
let detectedBpmAvailable = false;
let anchorOnlyDisplay = false;
let displaySourceLabel = "--";
let candidateRejectionReason = "--";
let mixDensity = 0;
let rejectedHighNoiseCount = 0;
let rejectedHighBpmCandidateCount = 0;
let rejectedSubdivisionCandidateCount = 0;
...
function updateLevelDisplay() {
  ...
  lowBandEnergyReadout.textContent = lowBandEnergy.toFixed(4);
  lowBandFluxReadout.textContent = lowBandFlux.toFixed(4);
...
function inferSelectedEstimator() {
  const source = displaySourceLabel.toLowerCase();
  if (source.includes("holding")) {
    return "held";
  }
  if (source.includes("target")) {
    return "target anchored";
  }
  if (source.includes("spectral")) {
    return "spectral";
  }
  if (source.includes("consensus")) {
    return "consensus";
  }
  return "onset";
}

function hasDetectedBpmEvidence(maxAgeMs = 1800) {
  const fresh = performance.now() - lastEstimatorUpdateTime <= maxAgeMs;
  return Boolean(fresh && (
    rawEstimatorBpm
    || spectralEstimatorBpm
    || onsetEstimatorBpm
    || estimatorAgreement > 0
  ));
}

function updateDetectionAvailabilityReadouts() {
  detectedBpmAvailable = hasDetectedBpmEvidence();
  anchorOnlyDisplay = Boolean(finalDisplayBpm && displaySourceLabel === "Target Anchor" && !detectedBpmAvailable);
  detectedBpmAvailableReadout.textContent = String(detectedBpmAvailable);
  anchorOnlyReadout.textContent = String(anchorOnlyDisplay);
  candidateRejectionReadout.textContent = candidateRejectionReason;
}

function setDisplaySource(label) {
  displaySourceLabel = label;
  displaySourceReadout.textContent = label;
...
function candidateRejectionFor(candidate, target = getTargetAnchorBpm()) {
  ...
  const bpm = candidate.bpm;
  const ratio = bpm / target;
  if (Math.abs(bpm - target * 2) <= Math.max(4, target * 0.04)) {
    return "double-time";
  }
  if (
    Math.abs(ratio - 1.5) <= 0.07
    || Math.abs(ratio - 1.75) <= 0.08
    || Math.abs(ratio - 5 / 3) <= 0.08
    || Math.abs(ratio - 7 / 4) <= 0.08
  ) {
    return "harmonic";
  }
  if (bpm > target * 1.45 || bpm < target * 0.62) {
    return "subdivision";
  }
  if ((candidate.aligned || 0) < 3 || (candidate.estimatorAgreement || 0) < 1) {
    return "insufficient beat-grid support";
  }
  return "accepted";
}

function isRejectableHighBpm(candidate, target = getTargetAnchorBpm()) {
  if (!candidate || !candidate.bpm) {
    return false;
  }
  if (candidate.bpm > 180) {
    return true;
  }
  return target ? candidate.bpm > target * 1.45 : false;
}

function recordCandidateRejection(candidate, reason = candidateRejectionFor(candidate)) {
  if (!candidate || !candidate.bpm || reason === "accepted") {
    return;
  }

  const key = `${Math.round(candidate.bpm)}:${reason}`;
  if (rejectedCandidateKeys.has(key)) {
    return;
  }
  rejectedCandidateKeys.add(key);

  if (isRejectableHighBpm(candidate)) {
    rejectedHighBpmCandidateCount += 1;
    rejectedHighBpmCandidatesReadout.textContent = String(rejectedHighBpmCandidateCount);
  }

  if (reason === "subdivision" || reason === "double-time" || reason === "harmonic") {
    rejectedSubdivisionCandidateCount += 1;
    rejectedSubdivisionCandidatesReadout.textContent = String(rejectedSubdivisionCandidateCount);
  }
}

function shouldRejectTargetCandidate(bpm, target = getTargetAnchorBpm()) {
  if (!target) {
    return false;
  }
  const anchor = targetAnchorStrength();
  if (Math.abs(bpm - target) <= anchor.range) {
    return false;
  }
  const reason = candidateRejectionFor({ bpm, aligned: 0, estimatorAgreement: 0 }, target);
  if (bpm > target * 1.45 || bpm > 180 || reason === "double-time" || reason === "harmonic") {
    recordCandidateRejection({ bpm }, reason);
    return true;
  }
  return false;
}
...
function currentAccuracyRow(marker = "") {
  ...
  const estimatorFresh = listening && tempoState !== "waiting" && now - lastEstimatorUpdateTime <= 1800;
  const displayFresh = listening && tempoState !== "waiting" && now - lastDisplayUpdateTime <= 2200;
  const finalBpm = displayFresh && finalDisplayBpm ? Number(finalDisplayBpm.toFixed(2)) : displayFresh && smoothedBpm ? Number(smoothedBpm.toFixed(2)) : null;
  const rawBpm = parseReadoutNumber(rawBpmReadout.textContent);
  const confidence = estimatorFresh ? parseReadoutNumber(confidenceReadout.textContent) || 0 : 0;
  if (firstEstimatorMetricTime === null && estimatorFresh && hasDetectedBpmEvidence(1800)) {
    firstEstimatorMetricTime = now - accuracyTestStartedAt;
  }
  if (firstConsensusMetricTime === null && displayFresh && displaySourceLabel === "Consensus" && detectedBpmAvailable) {
    firstConsensusMetricTime = now - accuracyTestStartedAt;
  }
  if (firstStableMetricTime === null && tempoState === "stable" && detectedBpmAvailable) {
    firstStableMetricTime = now - accuracyTestStartedAt;
  }
  return {
    ...
    marker,
    raw_detected_bpm: estimatorFresh ? rawEstimatorBpm || rawBpm : null,
    raw_estimator_bpm: estimatorFresh ? rawEstimatorBpm : null,
    spectral_bpm: estimatorFresh ? spectralEstimatorBpm : null,
    onset_bpm: estimatorFresh ? onsetEstimatorBpm : null,
    target_assisted_bpm: displaySourceLabel === "Target Anchor" ? target : estimatorFresh ? targetAssistedBpm : null,
    ...
    display_source: displayFresh ? displaySourceLabel : "none",
    selected_estimator: displayFresh ? inferSelectedEstimator() : "none",
    ...
    candidate_rejection_reason: estimatorFresh ? candidateRejectionReason : "--",
    top_5_candidates: estimatorFresh ? topCandidatesReadout.textContent : "--",
    ...
    rejected_high_bpm_candidates: rejectedHighBpmCandidateCount,
    rejected_subdivision_candidates: rejectedSubdivisionCandidateCount,
    mix_density: Number(mixDensity.toFixed(3)),
    low_band_energy: Number(lowBandEnergy.toFixed(5)),
...
function summarizeAccuracyTest() {
  ...
  const firstEstimatorMetric = accuracyTestRows.find((row) => Number.isFinite(row.time_to_first_estimator));
  const firstConsensusMetric = accuracyTestRows.find((row) => Number.isFinite(row.time_to_first_consensus));
  const firstStableMetric = accuracyTestRows.find((row) => Number.isFinite(row.time_to_stable));
...
function startAccuracyTest() {
  ...
  rejectedHighBpmCandidatesReadout.textContent = "0";
  rejectedSubdivisionCandidatesReadout.textContent = "0";
  if (tempoState === "waiting" || !listening) {
    lastEstimatorUpdateTime = 0;
    lastDisplayUpdateTime = 0;
    rawEstimatorBpm = null;
    onsetEstimatorBpm = null;
    spectralEstimatorBpm = null;
    targetAssistedBpm = null;
    estimatorAgreement = 0;
    detectedBpmAvailable = false;
    anchorOnlyDisplay = false;
    displaySourceLabel = "--";
...
function updateDisplayedBpm(correctedBpm, confidence, source) {
  ...
  finalDisplayBpm = smoothedBpm;
  lastDisplayUpdateTime = performance.now();
  finalDisplayBpmReadout.textContent = String(Math.round(finalDisplayBpm));
  const sourceLabel = source === "manual"
    ...
    : source === "target-anchor"
      ...
      : source === "spectral"
        ? "Spectral"
        : source === "onset" || source === "fallback peak" || source === "onset/grid"
          ? "Onset"
          : source === "holding"
            ? "Holding"
            : "Consensus";
  setDisplaySource(sourceLabel);
...
function addCandidate(candidates, bpm) {
  ...
  if (target) {
    ...
    if (shouldRejectTargetCandidate(bpm, target)) {
      return;
    }
...
function generateCandidateBpms(now) {
  ...
  if (target) {
    ...
    [target * 0.9, target * 0.95, target * 1.05, target * 1.1, target / 2, target * 2 / 3].forEach((candidate) => addCandidate(candidates, candidate));
  ...
  if (debugTarget) {
    [debugTarget, debugTarget * 0.9, debugTarget * 0.95, debugTarget * 1.05, debugTarget * 1.1, debugTarget / 2].forEach((candidate) => addCandidate(candidates, candidate));
  }
...
function spectralAutocorrelationCandidates(now) {
  ...
  if (target) {
    ...
    [target / 2, target * 0.9, target * 0.95, target * 1.05, target * 1.1].forEach((bpm) => candidateBpms.push(bpm));
  } else {
    const { min, max } = getBpmRange();
...
function buildTempoConsensus(enrichedScores, now, fastHistogram) {
  ...
  const target = getTargetAnchorBpm();
  const anchor = targetAnchorStrength();
  const rawGridCandidate = [...enrichedScores].sort((a, b) => b.score - a.score)[0] || null;
  const multiBandCandidate = rawGridCandidate;
  ...
  rawEstimatorBpm = rawGridCandidate?.bpm || null;
  onsetEstimatorBpm = intervalCandidate?.bpm || multiBandCandidate?.bpm || null;
  spectralEstimatorBpm = spectralCandidate?.bpm || null;
  targetAssistedBpm = targetCandidate?.bpm || (target ? target : null);
  ...
  function estimatorWeight(name, candidate, baseWeight) {
    ...
    const nearTarget = distance <= anchor.range;
    const relatedToTarget = Math.abs(candidate.bpm * 2 - target) <= anchor.range
      || Math.abs(candidate.bpm / 2 - target) <= anchor.range
      || Math.abs(candidate.bpm * 1.5 - target) <= anchor.range
  ...
  const consensusScores = enrichedScores.map((candidate) => {
    ...
    const estimatorConflict = target && targetAssistedBpm && rawEstimatorBpm
      ...
      : 0;
    const consensusConfidence = Math.min(100, Math.max(0, Math.round(
      (candidate.confidence || 0) * 0.36
      + Math.min(1, voteScore) * 31
      + Math.min(1, agreement / 3) * 22
      + lowMid * 10
      + (target && targetDistance <= anchor.range ? 13 : 0)
      + (spectralSupport > 0 ? 8 : 0)
      - estimatorConflict * 18
    )));
  ...
  rawEstimatorBpmReadout.textContent = rawEstimatorBpm ? String(Math.round(rawEstimatorBpm)) : "--";
  onsetBpmReadout.textContent = onsetEstimatorBpm ? String(Math.round(onsetEstimatorBpm)) : "--";
  targetAssistedBpmReadout.textContent = targetAssistedBpm ? String(Math.round(targetAssistedBpm)) : "--";
  ...
  estimatorAgreement = selected?.estimatorAgreement || 0;
  estimatorAgreementReadout.textContent = selected ? `${estimatorAgreement} estimators` : "--";
  updateDetectionAvailabilityReadouts();
...
function updateFastEstimate(now) {
  ...
  if (target) {
    ...
    const requiredRatio = anchorTarget ? 0.38 : 0.75;
  ...
  let prediction = predictionScore(best.bpm, now);
  if (anchorTarget && Math.abs(best.bpm - anchorTarget) > 5) {
    const targetPrediction = predictionScore(anchorTarget, now);
    if (targetPrediction.confidence >= Math.max(0.42, prediction.confidence * 0.72)) {
      best = { bpm: anchorTarget, score: best.score * (0.88 + targetPrediction.confidence * 0.24), targetAssisted: true };
  ...
  const spectralAgreement = spectralFast.some((candidate) => Math.abs(candidate.bpm - best.bpm) <= Math.max(3, best.bpm * 0.025)) ? 12 : 0;
...
function withConfidence(score, allScores) {
  ...
  const agreementBoost = Math.min(0.14, (score.estimatorAgreement || 0) * 0.04);
  const confidence = Math.min(100, Math.round((score.score * 0.38 + separation * 0.17 + alignment * 0.25 + coverage * 0.1 + agreementBoost) * 100));
...
function estimateTempo(now, source) {
  ...
  const targetBeforeEstimate = getTargetAnchorBpm();
  const anchorBeforeEstimate = targetAnchorStrength();
  const displayNearTarget = targetBeforeEstimate && isNearTarget(finalDisplayBpm || smoothedBpm, targetBeforeEstimate, anchorBeforeEstimate.range);
  ...
  if (activeBeats.length >= 8 && tempoState !== "reacquiring" && tempoState !== "stable" && tempoState !== "holding") {
    ...
    setTempoState(displayNearTarget ? "stabilizing" : smoothedBpm ? "reacquiring" : "acquiring");
  } else if (tempoState !== "stable" && tempoState !== "holding") {
    setTempoState(displayNearTarget ? "holding" : smoothedBpm ? "reacquiring" : "acquiring");
  }
  ...
  const enrichedScores = scores.map((score) => {
    ...
    const subdivisionPenalty = targetAnchor && targetDistance > anchor.range && (
      ...
      || Math.abs(score.bpm / 2 - targetAnchor) <= anchor.range
      || Math.abs(score.bpm * 1.5 - targetAnchor) <= anchor.range
      || Math.abs(score.bpm * 0.75 - targetAnchor) <= anchor.range
    ) ? 0.72 : 1;
    const unrelatedPenalty = targetAnchor && targetDistance > anchor.range ? anchor.outsidePenalty * subdivisionPenalty : 1;
    return {
      ...confidenceScore,
      historySupport: history.support,
      historySeconds: history.seconds,
      combinedScore: (score.score * 0.52 + history.support * 0.24 + fastBoost + targetBoost) * unrelatedPenalty
    };
  ...
  const topCandidateText = rankedScores.slice(0, 5).map((candidate) => {
    ...
    const rejection = candidateRejectionFor(candidate, targetAnchor);
    recordCandidateRejection(candidate, rejection);
    return `${Math.round(candidate.bpm)} s${Math.round(score * 100)} a${alignment}% k${Math.round((candidate.kickReliability || 0) * 100)}${estimators} ${candidateRelationship(candidate, choice.selected)} ${rejection}`;
  }).join(" | ");
  ...
  if (tempoState === "stable" || tempoState === "holding") {
    ...
    if (unstableCandidate) {
      disagreementBeats += 1;
      const disagreementLimit = targetHold ? 16 : denseHold ? 12 : 7;
      if (targetHold || denseHold || phase.agreement >= 0.46 || disagreementBeats < disagreementLimit) {
        ...
        finalDisplayBpm = lastStableBpm || smoothedBpm || targetAnchor;
        renderBpm(finalDisplayBpm);
        lastDisplayUpdateTime = now;
        finalDisplayBpmReadout.textContent = String(Math.round(finalDisplayBpm));
        setDisplaySource("Holding");
  ...
  const tightTargetConsensus = targetAnchor
    ...
    && choice.selected.estimatorAgreement >= 2
    && choice.selected.historySeconds >= 1.2;
  const realEstimatorEvidence = hasDetectedBpmEvidence(2500);
  if (realEstimatorEvidence && ((choice.selected.confidence >= profile.stableConfidence && activeBeats.length >= profile.stableBeats && choice.selected.historySeconds >= profile.stableSeconds && choice.selected.historySupport >= 0.32) || targetStableCandidate || tightTargetConsensus)) {
    setTempoState("stable");
    ...
    expectedBpm = corrected;
    lockReason = `holding near target consensus ${Math.round(corrected)} BPM; confidence ${choice.selected.confidence}%`;
    switchReasonReadout.textContent = lockReason;
  } else if (tempoState !== "holding" && (choice.selected.confidence >= 40 || (targetAnchor && isNearTarget(corrected, targetAnchor, anchor.range) && choice.selected.confidence >= 30)) && activeBeats.length >= 5) {
    setTempoState("stabilizing");
    holdStartTime = null;
    ...
    setTempoState(targetAnchor && isNearTarget(corrected, targetAnchor, anchor.range) ? "stabilizing" : smoothedBpm ? "reacquiring" : "acquiring");
    holdStartTime = null;
...
function handleFrame(frame) {
  ...
  const kickMedian = median(kickStrengthHistory);
  const kickMean = mean(kickStrengthHistory);
  const kickThreshold = Math.max(getNoiseFloor() * 0.35, kickMedian + kickMean * 0.52);
  const midThreshold = Math.max(getNoiseFloor() * 0.75, median(midStrengthHistory) + mean(midStrengthHistory) * 1.15);
  const lowPulseActivity = kickStrengthHistory.length
    ? kickStrengthHistory.filter((value) => value > kickThreshold).length / kickStrengthHistory.length
    : 0;
  const relativeLowRise = kickMean > 0 ? kickFlux / kickMean : 0;
  kickConfidence = Math.max(kickConfidence * 0.94, Math.min(1, lowPulseActivity * 0.58 + Math.min(1, relativeLowRise / 3) * 0.22));
...
function resetDetectionState() {
  ...
  rawEstimatorBpmReadout.textContent = "--";
  onsetBpmReadout.textContent = "--";
  multiBandBpmReadout.textContent = "--";
  spectralBpmReadout.textContent = "--";
  targetAnchorBpmReadout.textContent = "--";
  targetAssistedBpmReadout.textContent = "--";
  finalDisplayBpmReadout.textContent = "--";
  estimatorAgreementReadout.textContent = "--";
  detectedBpmAvailableReadout.textContent = "false";
  anchorOnlyReadout.textContent = "false";
  candidateRejectionReadout.textContent = "--";
  rejectedHighBpmCandidatesReadout.textContent = "0";
  rejectedSubdivisionCandidatesReadout.textContent = "0";
  ...
  lowBandEnergyReadout.textContent = "0.0000";
  lowBandFluxReadout.textContent = "0.0000";
...
function validateDomReferences() {
  const requiredIds = [
    ...
    "rawEstimatorBpmReadout",
    "onsetBpmReadout",
    "multiBandBpmReadout",
    "spectralBpmReadout",
    "targetAnchorBpmReadout",
    "targetAssistedBpmReadout",
    "finalDisplayBpmReadout",
    "estimatorAgreementReadout",
    "detectedBpmAvailableReadout",
    "anchorOnlyReadout",
    "candidateRejectionReadout",
    "rejectedHighBpmCandidatesReadout",
    "rejectedSubdivisionCandidatesReadout",
    "consensusReadout",
    "fastEstimateReadout",
    "stableCandidateReadout",
    "displaySourceReadout",
    "mixDensityReadout",
    "lowBandEnergyReadout",
    "lowBandFluxReadout",
